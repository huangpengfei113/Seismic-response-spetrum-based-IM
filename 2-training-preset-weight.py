...
# get preset weight from the structural response spectrum
def get_weight(path_stru_spect, acc_spec):
    stru_spec = read_csv_data(path_stru_spect)
    stru_pe = [float(i[0]) for i in stru_spec]
    stru_re = [float(i[1]) for i in stru_spec]
    ini_index = 0
    weight_all = []
    for pe_i in acc_spec:
        if pe_i < stru_pe[ini_index]:
            weight_all.append([0])
        elif pe_i >= stru_pe[-1]:
            weight_all.append([0])
        elif (pe_i >= stru_pe[ini_index]) and (pe_i <= stru_pe[ini_index+1]):
            weight_all.append([(pe_i - stru_pe[ini_index])/0.2*(stru_re[ini_index+1]-stru_re[ini_index])+stru_re[ini_index]])
        elif pe_i > stru_pe[ini_index+1]:
            ini_index += 1
            weight_all.append([(pe_i - stru_pe[ini_index]) / 0.2 * (stru_re[ini_index + 1] - stru_re[ini_index]) +
                               stru_re[ini_index]])
        else:
            raise ValueError
    return weight_all
...
def define_weight(shape, weight_all=K.zeros((798, 1))):
    new_weight = np.zeros(shape)
    for i in range(len(weight_all)):
        new_weight[i, 0] = weight_all[i][0]
    return new_weight
...
if __name__ == '__main__':
    ...
    my_weight = get_weight(r'E:\structural response spectrum.csv', [0.04+i*0.02 for i in range(798)])
    for try_i in range(30):
        ...
        for col_i, dm in zip([col1], ['col1']):
            for spec_i, im in zip([spec_acc], ['acc']):
                ...
                my_weight = define_weight((798, 1), weight_all=my_weight)
                spec_part1 = layers.Dense(1, kernel_initializer=keras.initializers.Constant(my_weight), activation=return_log, name='spec_mid', use_bias=False)(input_spec)
                spec_part4 = layers.Dense(1, kernel_initializer=keras.initializers.Constant([[0.5]]), bias_initializer=keras.initializers.Constant([[0.5]]), activation='linear', name='spec_out')(spec_part1)
    ...
    path_vali_loss = r'E:\vali_loss_all2.csv'  # path to save the loss value on test set
    write_csv(all_val_loss['col1-acc'], path_vali_loss)
    
    for key_i in all_val_loss.keys():
        path1 = os.path.join(r'E:\file to save average loss', key_i+'-valid2.csv')
        trans_csv(all_val_loss[key_i], ['average loss'], path1)
    for key_i in all_train_loss.keys():
        path1 = os.path.join(r'E:\file to save average loss', key_i+'-train2.csv')
